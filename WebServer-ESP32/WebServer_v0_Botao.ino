        ...
        if (c == '\n') {                    // Se o byte é um caractere de nova linha
          ...
          if (currentLine.length() == 0) {
            ...
            if (header.indexOf("GET /26/on") >= 0) {
              ...
              Serial.println("GPIO 02 ligado");
              output02State = "on";
              digitalWrite(output02, HIGH);
            } else if (header.indexOf("GET /02/off") >= 0) {
              Serial.println("GPIO 02 desligado");
              output02State = "off";
              digitalWrite(output02, LOW);
            }
            ...
            client.println(".button { background-color: #3065AC; border: none; color: white; padding: 16px 40px; border-radius: 6px; font-weight: bold; box-shadow: -1px 5px 5px #c2c2c2;");
            client.println("text-decoration: none; font-size: 30px; margin: 2px; cursor: pointer;}");
            client.println(".button2 {background-color: #D3D3D3;}</style></head>");
            ...
            // Mostrar estado atual e botões ON/OFF para GPIO 02
            client.println("<p>GPIO 2 - Estado " + output02State + "</p>");
            if (output02State=="off") {
              client.println("<p><a href=\"/02/on\"><button class=\"button\">LIGAR</button></a></p>");
            } else {
              client.println("<p><a href=\"/02/off\"><button class=\"button button2\">DESLIGAR</button></a></p>");
            } 
